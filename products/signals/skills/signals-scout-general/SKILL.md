...
description: >
  ...
  findings that clear the confidence bar via signals-scout-runs-findings-create.
  One peer in the signals-scout-* fleet — the cross-product explorer. Specialist
  siblings (signals-scout-llm-analytics, signals-scout-logs, etc.) carry tighter focus
...
- `signals-scout-scratchpad-list` — durable team steering inherited from past
...
- `signals-scout-runs-list` (last 7d) — what prior scouts found and ruled
  out. Skim summaries; pull `runs-retrieve` only when a summary mentions a topic
  you're considering.
- `signals-scout-project-profile-get` — deterministic snapshot (products in
...
- **Emit** via `signals-scout-runs-findings-create` if it clears the
