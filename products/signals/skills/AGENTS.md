...
2. **Scout fleet** — `signals-scout-*/`. Canonical default skills that the headless
   Signals scout loads into its system prompt at runtime. These are also the first
...
- `signals-scout-surveys/` — hybrid anomaly + theme-aggregation watcher for
...
For each team with an enabled `SignalScoutConfig`:

1. The coordinator builds the candidate set: `signals-scout-*` skills present on the
...
2. It samples `min(runs_per_tick, len(candidates))` skills uniformly at random
   without replacement.
3. Each sampled skill becomes one `RunSignalsScoutWorkflow` child run.
...
but contributes no runs that tick. See `scout_coordinator._resolve_skill_names_for_config`
...
- Coordinator + sampling rules — `../backend/temporal/agentic/scout_coordinator.py`
