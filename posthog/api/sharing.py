...
def _build_test_interview_share(access_token: str) -> Optional[SharingConfiguration]:
    ...
    from the topic UUID. The facade returns an unsaved SharingConfiguration whose
    ``interviewee_context.topic`` chain is pre-populated, so the rendering branch
    downstream handles this case unchanged.
    """
    from products.user_interviews.backend.facade.api import build_test_interview_share

    return build_test_interview_share(access_token)
